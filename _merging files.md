...
Here, under the s01 (subject 1) file we are accessing the s01 text file. Since this is a text file and not a csv (comma-separated-values), we indicate that the vales are separated by tabs not commas by using the escape character (\) with a t in the sep argument.
